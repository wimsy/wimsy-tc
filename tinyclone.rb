...
DataMapper.setup(:default, ENV['DATABASE_URL'] || 'mysql://root:miyaass@localhost/tinyclone')
...
class Link
  ...
  def self.create_link(original)
    ...
    if Link.first(:identifier => url.id.to_s(36)).nil? and !DIRTY_WORDS.include? url.id.to_s(36)
